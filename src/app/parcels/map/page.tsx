...
function ParcelsMapPageInner() {
  ...
  // ?dev=1 gate — restored for vista/vida/royale tuning only (the four
  // earlier heroes are already locked via HERO_*_ constants and read those
  // directly in the sync useEffect below). To kill dev-tool again once all
  // three are locked, restore the pre-tool sync useEffect from b596c81.
  const [devMode, setDevMode] = useState(false);
  useEffect(() => {
    if (typeof window === "undefined") return;
    const dev = new URLSearchParams(window.location.search).get("dev") === "1";
    setDevMode(dev);
    if (dev) console.log("[GLB] dev mode active (vista/vida/royale)");
  }, []);
  // Burj Vista 1 dev state — drag handle + 5 sliders.
  const [vistaYaw, setVistaYaw] = useState<number>(HERO_ORIENTATION_VISTA[1]);
  const [vistaPitch, setVistaPitch] = useState<number>(HERO_ORIENTATION_VISTA[0]);
  const [vistaRoll, setVistaRoll] = useState<number>(HERO_ORIENTATION_VISTA[2]);
  const [vistaSize, setVistaSize] = useState<number>(HERO_SIZE_SCALE_VISTA);
  const [vistaElev, setVistaElev] = useState<number>(HERO_COORDS_VISTA[2]);
  const [vistaLng, setVistaLng] = useState<number>(HERO_COORDS_VISTA[0]);
  const [vistaLat, setVistaLat] = useState<number>(HERO_COORDS_VISTA[1]);
  const [vistaHandlePx, setVistaHandlePx] = useState<{ x: number; y: number } | null>(null);
  const draggingVistaRef = useRef(false);
  // Vida Residence Downtown dev state.
  const [vidaYaw, setVidaYaw] = useState<number>(HERO_ORIENTATION_VIDA[1]);
  const [vidaPitch, setVidaPitch] = useState<number>(HERO_ORIENTATION_VIDA[0]);
  const [vidaRoll, setVidaRoll] = useState<number>(HERO_ORIENTATION_VIDA[2]);
  const [vidaSize, setVidaSize] = useState<number>(HERO_SIZE_SCALE_VIDA);
  const [vidaElev, setVidaElev] = useState<number>(HERO_COORDS_VIDA[2]);
  const [vidaLng, setVidaLng] = useState<number>(HERO_COORDS_VIDA[0]);
  const [vidaLat, setVidaLat] = useState<number>(HERO_COORDS_VIDA[1]);
  const [vidaHandlePx, setVidaHandlePx] = useState<{ x: number; y: number } | null>(null);
  const draggingVidaRef = useRef(false);
  // Burj Royale dev state.
  const [royaleYaw, setRoyaleYaw] = useState<number>(HERO_ORIENTATION_ROYALE[1]);
  const [royalePitch, setRoyalePitch] = useState<number>(HERO_ORIENTATION_ROYALE[0]);
  const [royaleRoll, setRoyaleRoll] = useState<number>(HERO_ORIENTATION_ROYALE[2]);
  const [royaleSize, setRoyaleSize] = useState<number>(HERO_SIZE_SCALE_ROYALE);
  const [royaleElev, setRoyaleElev] = useState<number>(HERO_COORDS_ROYALE[2]);
  const [royaleLng, setRoyaleLng] = useState<number>(HERO_COORDS_ROYALE[0]);
  const [royaleLat, setRoyaleLat] = useState<number>(HERO_COORDS_ROYALE[1]);
  const [royaleHandlePx, setRoyaleHandlePx] = useState<{ x: number; y: number } | null>(null);
  const draggingRoyaleRef = useRef(false);
...
// Hover handlers — the LAYER_REGISTRY loader registers per-layer
    ...
    overlay.setProps({
      layers: [
        ...
        new ScenegraphLayer({
          ...
          data: [{ position: [vistaLng, vistaLat, vistaElev] as [number, number, number] }],
          scenegraph: HERO_GLB_URL_VISTA,
          getPosition: (d: { position: [number, number, number] }) => d.position,
          getOrientation: [vistaPitch, vistaYaw, vistaRoll],
          sizeScale: vistaSize,
          _lighting: "pbr",
          pickable: false,
        ...
        new ScenegraphLayer({
          ...
          data: [{ position: [vidaLng, vidaLat, vidaElev] as [number, number, number] }],
          scenegraph: HERO_GLB_URL_VIDA,
          getPosition: (d: { position: [number, number, number] }) => d.position,
          getOrientation: [vidaPitch, vidaYaw, vidaRoll],
          sizeScale: vidaSize,
          _lighting: "pbr",
          pickable: false,
        ...
        new ScenegraphLayer({
          id: "hero-burj-royale",
          data: [{ position: [royaleLng, royaleLat, royaleElev] as [number, number, number] }],
          scenegraph: HERO_GLB_URL_ROYALE,
          getPosition: (d: { position: [number, number, number] }) => d.position,
          getOrientation: [royalePitch, royaleYaw, royaleRoll],
          sizeScale: royaleSize,
          _lighting: "pbr",
          pickable: false,
      ...
      vistaLng, vistaLat, vistaYaw, vistaPitch, vistaRoll, vistaSize, vistaElev,
      vidaLng, vidaLat, vidaYaw, vidaPitch, vidaRoll, vidaSize, vidaElev,
      royaleLng, royaleLat, royaleYaw, royalePitch, royaleRoll, royaleSize, royaleElev]);

  // Handle projections — project lng/lat → screen-space pixel for the gold
  // crosshair drag handles. One useEffect per building so each handle
  // tracks its own anchor through map move.
  useEffect(() => {
    const map = mapRef.current;
    if (!map || !devMode) return;
    const project = () => {
      const p = map.project([vistaLng, vistaLat]);
      setVistaHandlePx({ x: p.x, y: p.y });
    };
    project();
    map.on("move", project);
    return () => { map.off("move", project); };
  }, [vistaLng, vistaLat, mapStyleReady, devMode]);
  useEffect(() => {
    const map = mapRef.current;
    if (!map || !devMode) return;
    const project = () => {
      const p = map.project([vidaLng, vidaLat]);
      setVidaHandlePx({ x: p.x, y: p.y });
    };
    project();
    map.on("move", project);
    return () => { map.off("move", project); };
  }, [vidaLng, vidaLat, mapStyleReady, devMode]);
  useEffect(() => {
    const map = mapRef.current;
    if (!map || !devMode) return;
    const project = () => {
      const p = map.project([royaleLng, royaleLat]);
      setRoyaleHandlePx({ x: p.x, y: p.y });
    };
    project();
    map.on("move", project);
    return () => { map.off("move", project); };
  }, [royaleLng, royaleLat, mapStyleReady, devMode]);

  // Drag-handle pointer-down — disable map dragPan, capture move at document
  // level so the cursor can wander, restore on mouseup.
  const onVistaHandleMouseDown = (e: React.MouseEvent) => {
    e.preventDefault(); e.stopPropagation();
    const map = mapRef.current; if (!map) return;
    map.dragPan.disable(); draggingVistaRef.current = true;
    const onMove = (ev: MouseEvent) => {
      const rect = map.getContainer().getBoundingClientRect();
      const ll = map.unproject([ev.clientX - rect.left, ev.clientY - rect.top]);
      setVistaLng(ll.lng); setVistaLat(ll.lat);
    };
    const onUp = () => {
      map.dragPan.enable(); draggingVistaRef.current = false;
  ...
  const onVidaHandleMouseDown = (e: React.MouseEvent) => {
    e.preventDefault(); e.stopPropagation();
    const map = mapRef.current; if (!map) return;
    map.dragPan.disable(); draggingVidaRef.current = true;
    const onMove = (ev: MouseEvent) => {
      const rect = map.getContainer().getBoundingClientRect();
      const ll = map.unproject([ev.clientX - rect.left, ev.clientY - rect.top]);
      setVidaLng(ll.lng); setVidaLat(ll.lat);
    };
    const onUp = () => {
      map.dragPan.enable(); draggingVidaRef.current = false;
      document.removeEventListener("mousemove", onMove);
      document.removeEventListener("mouseup", onUp);
    };
    document.addEventListener("mousemove", onMove);
  ...
  const onRoyaleHandleMouseDown = (e: React.MouseEvent) => {
    e.preventDefault(); e.stopPropagation();
    const map = mapRef.current; if (!map) return;
    map.dragPan.disable(); draggingRoyaleRef.current = true;
    const onMove = (ev: MouseEvent) => {
      const rect = map.getContainer().getBoundingClientRect();
      const ll = map.unproject([ev.clientX - rect.left, ev.clientY - rect.top]);
      setRoyaleLng(ll.lng); setRoyaleLat(ll.lat);
    };
    const onUp = () => {
      map.dragPan.enable(); draggingRoyaleRef.current = false;
      document.removeEventListener("mousemove", onMove);
      document.removeEventListener("mouseup", onUp);
    };
    document.addEventListener("mousemove", onMove);
  ...
  // Copy Config helpers — writes drop-in HERO_COORDS_* / orientation / size
  // snippet to clipboard so founder can paste back into source to lock.
  const copyVistaConfig = () => {
    const text = `// Burj Vista 1\ndata: [{ position: [${vistaLng.toFixed(6)}, ${vistaLat.toFixed(6)}, ${vistaElev}] }],\ngetOrientation: [${vistaPitch}, ${vistaYaw}, ${vistaRoll}],\nsizeScale: ${vistaSize},`;
    try { void navigator.clipboard.writeText(text); } catch { /* clipboard blocked */ }
  };
  const copyVidaConfig = () => {
    const text = `// Vida Residence Downtown\ndata: [{ position: [${vidaLng.toFixed(6)}, ${vidaLat.toFixed(6)}, ${vidaElev}] }],\ngetOrientation: [${vidaPitch}, ${vidaYaw}, ${vidaRoll}],\nsizeScale: ${vidaSize},`;
    try { void navigator.clipboard.writeText(text); } catch { /* clipboard blocked */ }
  };
  const copyRoyaleConfig = () => {
    const text = `// Burj Royale\ndata: [{ position: [${royaleLng.toFixed(6)}, ${royaleLat.toFixed(6)}, ${royaleElev}] }],\ngetOrientation: [${royalePitch}, ${royaleYaw}, ${royaleRoll}],\nsizeScale: ${royaleSize},`;
    try { void navigator.clipboard.writeText(text); } catch { /* clipboard blocked */ }
  };
  ...
  return (
    ...
    >
      ...
      {/* ── Dev-tool — gold crosshair drag handles + control panels for
          vista/vida/royale tuning. Gated behind ?dev=1; the 3 panels are
          right:16 / right:290 / right:564. Founder workflow: drag handle
          to reposition, scrub sliders for orientation/size/elev, then
          Copy Config to clipboard and paste the values into the
          HERO_*_VISTA/VIDA/ROYALE constants to lock. */}
      {devMode && vistaHandlePx && glbActive && (
        <div onMouseDown={onVistaHandleMouseDown} title="Drag to move Burj Vista 1 anchor"
          style={{ position: "absolute", left: vistaHandlePx.x - 12, top: vistaHandlePx.y - 12,
            width: 24, height: 24, cursor: draggingVistaRef.current ? "grabbing" : "grab",
            border: "2px solid #C8A96E", borderRadius: "50%",
            background: "rgba(200,169,110,0.2)",
            boxShadow: "0 0 0 1px rgba(0,0,0,0.4), 0 0 8px rgba(200,169,110,0.6)",
            zIndex: 50, pointerEvents: "auto" }} />
      )}
      {devMode && vidaHandlePx && glbActive && (
        <div onMouseDown={onVidaHandleMouseDown} title="Drag to move Vida anchor"
          style={{ position: "absolute", left: vidaHandlePx.x - 12, top: vidaHandlePx.y - 12,
            width: 24, height: 24, cursor: draggingVidaRef.current ? "grabbing" : "grab",
            border: "2px solid #C8A96E", borderRadius: "50%",
            background: "rgba(200,169,110,0.2)",
            boxShadow: "0 0 0 1px rgba(0,0,0,0.4), 0 0 8px rgba(200,169,110,0.6)",
            zIndex: 50, pointerEvents: "auto" }} />
      )}
      {devMode && royaleHandlePx && glbActive && (
        <div onMouseDown={onRoyaleHandleMouseDown} title="Drag to move Burj Royale anchor"
          style={{ position: "absolute", left: royaleHandlePx.x - 12, top: royaleHandlePx.y - 12,
            width: 24, height: 24, cursor: draggingRoyaleRef.current ? "grabbing" : "grab",
            border: "2px solid #C8A96E", borderRadius: "50%",
            background: "rgba(200,169,110,0.2)",
            boxShadow: "0 0 0 1px rgba(0,0,0,0.4), 0 0 8px rgba(200,169,110,0.6)",
            zIndex: 50, pointerEvents: "auto" }} />
      ...
      {devMode && (
        <DevToolPanel title="BURJ VISTA 1" right={16}
          lng={vistaLng} lat={vistaLat}
          yaw={vistaYaw} pitch={vistaPitch} roll={vistaRoll}
          size={vistaSize} elev={vistaElev}
          onYaw={setVistaYaw} onPitch={setVistaPitch} onRoll={setVistaRoll}
          onSize={setVistaSize} onElev={setVistaElev}
          onCopy={copyVistaConfig} />
      )}
      {devMode && (
        <DevToolPanel title="VIDA RESIDENCE" right={290}
          lng={vidaLng} lat={vidaLat}
          yaw={vidaYaw} pitch={vidaPitch} roll={vidaRoll}
          size={vidaSize} elev={vidaElev}
          onYaw={setVidaYaw} onPitch={setVidaPitch} onRoll={setVidaRoll}
          onSize={setVidaSize} onElev={setVidaElev}
          onCopy={copyVidaConfig} />
      )}
      {devMode && (
        <DevToolPanel title="BURJ ROYALE" right={564}
          lng={royaleLng} lat={royaleLat}
          yaw={royaleYaw} pitch={royalePitch} roll={royaleRoll}
          size={royaleSize} elev={royaleElev}
          onYaw={setRoyaleYaw} onPitch={setRoyalePitch} onRoll={setRoyaleRoll}
          onSize={setRoyaleSize} onElev={setRoyaleElev}
          onCopy={copyRoyaleConfig} />
      )}
...
// Dev-tool panel — gold glassmorphism, gated by ?dev=1. Used by 3 active
// heroes (vista/vida/royale). 5 sliders + Copy Config button. Reusable so
// adding/removing a hero stays a single JSX block, not 150 lines per.
