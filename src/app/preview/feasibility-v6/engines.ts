...
//
// Diff baselines: every engine pre-records its own default psf values. The
// preview compares the user's current input vs the engine's default and shows
// a 4-tone delta badge per 03_UX_FULLSCREEN_AND_DIFF.md spec.
...
// 4-tone diff threshold per 03_UX_FULLSCREEN_AND_DIFF.md
export type DiffTone = 'green' | 'amber' | 'amberBold' | 'red';

export function diffTone(currentValue: number, defaultValue: number): {
  tone: DiffTone;
  pct: number;
  color: string;
  label: string;
