...
export function installZaahiThreeLayer(
  ...
  const customLayer: maplibregl.CustomLayerInterface = {
    ...
    render(_gl, args: unknown) {
      ...
      // ── MapLibre v5 CustomLayer compatibility shim (2026-06-12) ──
      // MapLibre v4 passed a flat 16-element matrix as the 2nd arg.
      // MapLibre v5 changed this to a `CustomRenderMethodInput` object:
      //   { defaultProjectionData: { mainMatrix, ... },
      //     modelViewProjectionMatrix, projectionMatrix, shaderData,
      //     farZ, nearZ, fov, ... }
      // The old code cast `matrix as unknown as ArrayLike<number>` →
      // `new THREE.Matrix4().fromArray(matrix)` silently produced
      // garbage values when the cast hit an object (no .length, all
      // entries undefined → NaN matrix → everything outside clip space
      // → silent no-render). This is the root cause of the 5-iteration
      // "flat grey then totally blank" investigation log in
      // docs/research/signature-realistic-2026-06-10.md §10.
      //
      // STEP 0 of the §10 protocol (rebuild official MapLibre example
      // from scratch) confirmed the issue lives at exactly this seam:
      // the OFFICIAL minimal example also doesn't render the cube
      // without this unwrap. None of the six §10 hypotheses (1-6) are
      // the cause; they were all premised on the matrix arriving
      // correctly.
      ...
      if (args && (Array.isArray(args) || args instanceof Float32Array || args instanceof Float64Array)) {
        ...
        if (cand) {
          if (Array.isArray(cand) || cand instanceof Float32Array || cand instanceof Float64Array) {
            ...
            // MapLibre serialises mat4 entries as indexed-object {0..15}.
