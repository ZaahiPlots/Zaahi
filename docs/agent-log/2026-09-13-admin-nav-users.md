...
### Outcome
- Feature commit `24f51a4` on `feat/admin-nav-users`.
- `git merge --no-ff` into `main` hit one conflict: `docs/agent-log/00-index.md` (main's docs commit
  `8267dbe` had rewritten the pause-subscription row while this branch appended the nav row).
  Resolved by keeping main's row and appending the nav row. The first `git push` in the same chain
  ran before the conflict was resolved and pushed only `8267dbe` (the docs commit that was
  pre-approved to ride along); the merge commit was pushed right after.
