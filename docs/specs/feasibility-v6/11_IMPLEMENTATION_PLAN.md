...
### Sprint 9 — PDF polish (4 h)
- Extend v5 jsPDF report: add engine name + diff-summary section + parcel header strip + footer with feasibility version + scenario name.
- Cover page brand styled per FEASIBILITY_STYLE_GUIDE §6.1 (Georgia ZAAHI 44 px, gold rule line).
- One PDF for all 13 engines — engine-specific sections rendered conditionally.
- **Smoke:** PDF generates < 800 ms on a single engine · file size < 500 KB · all currency formatted with `toLocaleString` thousand separators · pnpm build clean.
- **Founder check:** YES — Dymo reviews the PDF on a real parcel before next sprint.
