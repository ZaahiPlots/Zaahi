/**
 * Download all Dubai Hills plot polygons from DDA layer 2 (BASIC_LAND_BASE)
 * and save as ~/zaahi/data/layers/dda/dubai_hills.geojson.
 *
 * Source is public — no token required. Pagination by OBJECTID with 500ms pause.
 *
 * Run: npx tsx scripts/build-dubai-hills.ts
 */
...
const OUT_PATH = join(DDA_DIR, 'dubai_hills.geojson');
...
async function fetchPage(offset: number): Promise<EsriPage> {
  const params = new URLSearchParams({
    where: "PROJECT_NAME='DUBAI HILLS'",
    ...
    for (const f of data.features) {
      ...
      features.push({
        ...
        properties: {
          ...
          PROJECT_NAME: f.attributes.PROJECT_NAME ?? 'DUBAI HILLS',
