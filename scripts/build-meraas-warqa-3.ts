/**
 * Download all MERAAS PLOTS AT AL WARQA'A THIRD plot polygons from DDA layer 2
 * Run: npx tsx scripts/build-meraas-warqa-3.ts
 */
...
// SQL-escape the apostrophe by doubling it
const PROJECT_NAME_SQL = "MERAAS PLOTS AT AL WARQA''A THIRD";
const PROJECT_NAME = "MERAAS PLOTS AT AL WARQA'A THIRD";
const PAGE = 2000;
const PAUSE_MS = 500;

const DDA_DIR = join(homedir(), 'zaahi', 'data', 'layers', 'dda');
const OUT_PATH = join(DDA_DIR, 'meraas_warqa_3.geojson');
...
async function fetchPage(offset: number): Promise<EsriPage> {
  const params = new URLSearchParams({
    where: `PROJECT_NAME='${PROJECT_NAME_SQL}'`,
