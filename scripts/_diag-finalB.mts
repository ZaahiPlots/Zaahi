...
for (const r of Object.values(real)) {
  ...
  const inside = (k: number) => [[-1, -1], [1, -1], [1, 1], [-1, 1]].every(([su, sv]) => { const u = su * (fitW / 2) * k, v = sv * (fitD / 2) * k; return pir(0 + u * c - v * s, 0 + u * s + v * c, plotM); });
  ...
  const centerOff = 0; // centroid-centred
  if (centerOff > 3) t.centerBad++;
  // 2. SIZE overhang (after clamp)
  const cornersOut = [[-1, -1], [1, -1], [1, 1], [-1, 1]].filter(([su, sv]) => { const u = su * (fitW / 2), v = sv * (fitD / 2); return !pir(0 + u * c - v * s, 0 + u * s + v * c, plotM); }).length;
